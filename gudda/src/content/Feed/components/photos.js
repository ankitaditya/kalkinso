  ...
  return images.map(
    ({src, alt}, index) =>
      ({
        src,
        alt,
        width: breakpoints[index],
        height: Math.round((breakpoints[index+1] / breakpoints[index]) * breakpoints[index]),
        srcSet: breakpoints.map((breakpoint) => ({
          src,
          width: breakpoint,
          height: Math.round((breakpoints[index+1] / breakpoints[index]) * breakpoint),
        })),
      }),
  )
}
